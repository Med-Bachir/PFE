const {
  ...
  verifyTokenAndAuthorizationA_C,
  verifyTokenAndAdminandSeller,
  verifyToken,
  ...
  async (req, res) => {
    try {
      ...
      const checkExistingRequestQuery =
        "SELECT * FROM SHOP WHERE id_Owner = ? AND status = 'close'";
      ...
      connection.query(
        ...
        checkExistingRequestValues,
        (err, existingRequestResult) => {
          if (err) {
            console.error("Error checking existing request:", err);
            res
              .status(500)
              .json({ error: "Failed to check existing request." });
            return;
          }
          ...
          if (existingRequestResult.length > 0) {
            ...
            res
              ...
              .json({
                error:
                  "You have already submitted a shop creation request. Please wait for admin approval.",
              });
            return;
          }
          ...
          const insertLocationQuery =
            "INSERT INTO LOCATION (country, state, city, street) VALUES (?, ?, ?, ?)";
          ...
          connection.query(
            ...
            (err, locationResult) => {
              ...
              const insertShopQuery =
                "INSERT INTO SHOP (id_Owner, shopname, shopimage, shopdesc, status, shopcover, number, id_Location, createdAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW())";
              const insertShopValues = [
                id_Owner,
                shopname,
                uploadResultImage.secure_url,
                shopdesc,
                "Waiting",
                uploadResultCover.secure_url,
                number,
                id_Location,
              ];
                  ...
                  connection.query(
                    ...
                    (err, userResult) => {
                      if (err) {
                        ...
                        res
                          .status(500)
                          .json({ error: "Failed to fetch username." });
                        return;
                      }
                      ...
                      const insertNotificationQuery =
                        "INSERT INTO NOTIFICATION (text, type, reciver) VALUES (?, ?, ?)";
        ...
        (err, sellerResult) => {
          ...
          connection.query(
            ...
            (err, notificationResult) => {
              ...
              res
                .status(200)
                .json({ message: "Seller approved successfully." });
            }
          );
        }
      );
...
router.get(
  "/shop-approval-notifications/:reciver",
  verifyTokenAndSeller,
  async (req, res) => {
    try {
      ...
      const getNotificationsQuery =
        "SELECT * FROM NOTIFICATION WHERE reciver = ?";
      ...
      (err, shopOwnerResult) => {
        ...
        connection.query(deleteShopQuery, [shopId], (err, deleteResult) => {
          ...
          const notificationText =
            "Your shop application has been refused by the admin.";
          const insertNotificationQuery =
            "INSERT INTO NOTIFICATION (text, type, reciver) VALUES (?, ?, ?)";
          const insertNotificationValues = [
            notificationText,
      ...
      connection.query(
        ...
        (err, notificationResult) => {
          ...
          const deleteNotificationQuery =
            "DELETE FROM NOTIFICATION WHERE idNOTIFICATION = ?";
          connection.query(
          ...
          connection.query(deleteShopQuery, [shopId], (err, result) => {
            ...
            connection.commit((err) => {
              ...
              res
                ...
                .json({
                  message:
                    "Shop and associated stock and products deleted successfully.",
                });
            });
          });
        });
