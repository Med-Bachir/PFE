...
//TOP 10 RATED PRODUCTS FOR ALL SHOPS OF A SPECIFIC SELLER
router.get("/top-rated-products/:sellerId",verifyTokenAndAuthorizationA_S ,async (req, res) => {
  try {
      ...
      connection.query(getTopRatedProductsQuery, [sellerId], (err, products) => {
          if (err) {
              console.error("Error retrieving top rated products:", err);
              res.status(500).json({ error: "Failed to retrieve top rated products." });
              return;
...
router.get("/top-sold-products-admin", verifyTokenAndAdmin, async (req, res) => {
  try {
...
router.get("/top-sold-products/:sellerId", verifyTokenAndAuthorizationA_S, async (req, res) => {
  try {
      const sellerId = req.params.sellerId;
      ...
      connection.query(topSoldProductsQuery, [sellerId], (err, results) => {
          if (err) {
              ...
              res.status(500).json({ error: "Failed to fetch top sold products for seller." });
              return;
          }
...
router.get("/orders/status-count", verifyTokenAndAdmin, async (req, res) => {
  ...
  router.get("/seller-stats/:sellerId", verifyTokenAndSeller, async (req, res) => {
    const sellerId = req.params.sellerId; 
    try {
        const query = `
        WITH OwnerProducts AS (
            ...
            JOIN ecommerce.stock st ON p.idPRODUCT = st.id_Product
            JOIN ecommerce.shop sh ON st.id_Shop = sh.idSHOP
            JOIN ecommerce.user o ON sh.id_Owner = ${connection.escape(sellerId)}
        )
        ...
        SELECT 
            (SELECT COUNT(*) 
             ...
             WHERE sh.id_Owner = ${connection.escape(sellerId)}) AS totalProducts,
